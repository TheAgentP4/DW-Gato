  ...
  <span class="fixed flex top-0 left-0 w-full h-full bg-black bg-opacity-70 justify-center items-center" v-if="ganador !== null">
    <span class="flex flex-col justify-center items-center bg-black-ice p-3 shadow-lg h-1/4 w-full">
      <div class="flex flex-col justify-center items-center h-full w-[300px] m-2">
        ...
        <span class="flex flex-row justify-center items-center w-full h-full">
          <button class="flex flex-col justify-center items-center bg-blue-dark rounded-lg p-1 m-1 w-1/3 text-black font-bold hover:bg-white transition-all duration-500 ease-in-out" @click="$emit('reiniciar-juego')">SALIR</button>
          <button class="flex flex-col justify-center items-center bg-blue-dark rounded-lg p-1 m-1 w-1/3  text-black  font-bold hover:bg-yellow-light " @click="$emit('inicio')">REINICIAR</button>
        </span>
      </div>
    </span>
  </span>
